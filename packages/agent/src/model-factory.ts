 ...
 * This LangChain client still uses Chat Completions, so force "none" for that family.
 */
function needsChatCompletionsReasoningNone(model: string): boolean {
  return /gpt-5\.6/i.test(model);
}

function openaiReasoningOpts(config: LlmConfig): Record<string, unknown> {
...
export function createChatModel(config: LlmConfig): any {
  ...
  const reasoning = openaiReasoningOpts(config);
