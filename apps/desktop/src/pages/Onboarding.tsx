  ...
  async function saveAndFinish() {
    ...
    try {
      await api('settings_set', {
        settings: {
          ...settings,
          pulseConfigured: Boolean(settings.pulseEmail),
          llmConfigured: Boolean(llmApiKey) || settings.llmConfigured,
    ...
    <div style={{ minHeight: '100vh', background: '#0b1220', color: '#e2e8f0', padding: 48 }}>
      ...
      {step === 0 && (
        <div style={{ maxWidth: 480 }}>
          ...
          <input
            style={inputStyle}
            value={settings.pulseSupabaseUrl || ''}
            onChange={(e) => setSettings({ ...settings, pulseSupabaseUrl: e.target.value })}
          />
          <label style={labelStyle}>Supabase Anon Key</label>
          <input
            style={inputStyle}
            value={settings.pulseSupabaseAnonKey || ''}
            onChange={(e) => setSettings({ ...settings, pulseSupabaseAnonKey: e.target.value })}
          />
      ...
      {step === 1 && (
        <div style={{ maxWidth: 480 }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
            ...
            <button style={btnStyle} onClick={() => void saveAndFinish()}>Finish Setup</button>
          </div>
        </div>
      )}

      {status && <p style={{ color: status.startsWith('Saving') ? '#94a3b8' : '#f87171' }}>{status}</p>}
    </div>
  );
}
